...
double X,Y,Z,dx,dy,dz,t,dt,nsteps,epsilon,RelTol,dcd,sha,idmi,rad;
...
int main(void)
{
	...
	void (*solver)(double ***u, double ***u1,double ***F,
	unsigned long , unsigned long ,unsigned long,
	double,double,double,double);
	
	double Adtx,Adty,Adtz,Bdtx,Bdty,Bdtz;
	double norm,en,stray,exch,en0,ani,dmi;
   ...
   //***Stray field Hs=(h1,h2,h3)
	...
	//***Generating or loading kernels for stray field
	...
	writetoovf(m1,m2,m3,dx,dy,dz,0);
	...
	for (count=1;count<=nsteps;count++){
		...
		/**Solve heat equation (with source) subject to chosen boundary condition**/
      ...
      effective(m1,m2,m3,h1,h2,h3,
	   ...
	   if(fmod(count,nsteps/(npoints+0.0))==0.0){
		   en0=en;
	   	   energy(&en,&stray,&exch,&ani,&dmi,m1,m2,m3,h1,h2,h3,he1,he2,he3);
                   output(m1,m2,m3,en,stray,exch,ani,dmi);
				   writetoovf(m1,m2,m3,dx,dy,dz,count);
